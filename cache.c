...
cache_create(char *name,		/* name of the cache */
         ...
         unsigned int MSHR_enabled,
         unsigned int MSHR_n,
         unsigned int MSHR_n_targets)
{
  struct cache_t *cp;
  struct cache_blk_t *blk;
  int i, j, bindex;
  ...
  /* initialize MSHRs */
  cp->MSHR_enabled = MSHR_enabled;
  if (MSHR_enabled) {
    cp->MSHR_n = MSHR_n;
    cp->MSHR_n_targets = MSHR_n_targets;
    cp->MSHR_allocated = 0;
    cp->MSHR = (struct mshr_t *)calloc(MSHR_n, sizeof(struct mshr_t));
    if (!cp->MSHR)
      fatal("out of virtual memory");
  }
...
//    fatal("cache: access error: access spans block, addr 0x%08x", addr);
  ...
  /* check if secondary miss(MSHR hit) */
  /* also, refresh MSHR status */
  for (i=0; i<cp->MSHR_n; i++) {
    mshr = &cp->MSHR[i];
    if (mshr->valid) {
      /* found an entry that should be invalidated */
      if (mshr->ready <= now) {
        mshr->valid = FALSE;
        cp->MSHR_allocated--;
      }
      /* found MSHR hit, abandon refresh */
      else if (mshr->tagset == CACHE_TAGSET(cp, addr)) {
        goto mshr_mshr_hit;
      }
    }
  }
  /* no MSHR hit, MSHR refreshed */
  ...
  /* check if space is left to handle this miss */
  if (cp->MSHR_allocated < cp->MSHR_n) {
    for (i=0; i<cp->MSHR_n; i++) {
      mshr = &cp->MSHR[i];
      if (!mshr->valid)
        break;
    ...
    mshr->valid = TRUE;
    mshr->tagset = CACHE_TAGSET(cp, addr);
    mshr->ready = now+lat;
    mshr->num_targets = 1;
    return lat;
  }
 ...
 mshr_mshr_hit: /* secondary miss (MSHR hit) handler */
  
  /* check if space is left in mshr */
  if (mshr->num_targets < cp->MSHR_n_targets) {
    /* allocate space for this miss */
    mshr->num_targets++;
    if (mshr->ready - now <= 0)
      panic("mshr is already ready");
    return BOUND_POS(mshr->ready - now);
  }
  else { 
    /* structural miss, cannot service */
    return CACHE_BLOCKED;
  }


 mshr_cache_hit: /* slow hit handler */
  ...
  /* cache hit latency, don't have to take care of secondary misses */
  return cp->hit_latency;

 mshr_cache_fast_hit: /* fast hit handler */
