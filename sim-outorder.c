...
/* data types based on host and compiler */
#include "host.h"

/* functions for runtime messages, random, strings, etc. */
#include "misc.h"

/* macros for defining the instructions in the ISA and slicing instructions, struct/enum for registers */
#include "machine.h"

/* struct regs_t, regs_create() and regs_init() */
#include "regs.h"

/* 
 * inverted page table, flat mem, and mem access/manipulation
 * mem_access_fn: memory access function pointer type
 */
#include "memory.h"

/* 
 * whether to allocate the data part is configurable too
 * cache_blk_t -> cache_set_t -> cache_t
 * cache_access(): returns cache access cycle
 * blk_access_fn: block miss/replacement function pointer type, returns access latency
 * way chain inside each set is a DLL that connects blocks
 */
#include "cache.h"

/* load the binary into a simulated virtual memory space */
#include "loader.h"

/* implements the Alpha ISA system calls */
#include "syscall.h"

/*
 * BPredComb, BPred2Level, BPred2bit + stateless
 * bpred_btb_ent_t: BTB entry, has lru chaining pointers
 * bpred_dir_t: direction predictor, supports history xor address with amount of history defined by SHIFT_WIDTH
 * bpred_t: branch predictor, supports tournament prediction where dirpred.meta chooses between dirpred.bimod and dirpred.twolev
 */
#include "bpred.h"

/*
 * res_desc: a class of resource (e.g. integer-ALU), can have up to 16 res_templates 
 * res_pool: a collection all resources, with one entry per resource instance, table[class][i] = ith instance of class
 * free resource is found from res_pool by res_get()
 */
#include "resource.h"

/*
 * macros for bits
 * __BITMAP_OP(dest, src1, src2, size, operation)
 */
#include "bitmap.h"

/* handles commandline arguments by creating an option database */
#include "options.h"

/* evaluate expressions in string, as python eval() does */
#include "eval.h"

/*
 * statistics, uses expression eval
 * creates a statistical database, where statistical variables (int, float, distrubution, ...) are registered
 */
#include "stats.h"

/* pipeline tracing, specified by range in commandline */
#include "ptrace.h"
...
/* 
 * simulator interface, including execution time, init, etc.
 * 01. register options                                              (sim_reg_options())
 * 02. parse options                                                 (in main())
 * 03. check simulator-specific options                              (sim_ckeck_options())
 * 04. register simulator-specific statistic                         (sim_reg_stats())
 * 05. initialize simulator                                          (sim_init())
 * 06. print option values                                           (in main())
 * 07. print simulator config info                                   (sim_aux_config())
 * 08. load program, initialize precise state, and begin simulation  (sim_main())
 * 09. print stat values                                             (in main())
 * 10. dump simulator stats                                          (sim_aux_stats())
 * 11. un-initialize simulator                                       (sim_uninit())
 * 12. print all simulator stats                                     (sim_print_stats())
 */
#include "sim.h"
...
/* FMT/sFMT global counters */
static counter_t FMT_itlb_penalty = 0;
static counter_t FMT_il1_penalty = 0;
static counter_t FMT_il2_penalty = 0;
static counter_t FMT_dtlb_penalty = 0;
static counter_t FMT_dl1_penalty = 0;
static counter_t FMT_dl2_penalty = 0;
...
struct fmt_ent_t {
  ...
  counter_t local_il1_penalty;
  counter_t local_il2_penalty;
...
sim_reg_stats(struct stat_sdb_t *sdb)   /* stats database */
{
  ...
  /* FMT/sFMT performance stats */
  stat_reg_counter(sdb, "FMT_itlb",
           "FMT: itlb miss penalty cycles",
           &FMT_itlb_penalty, /* initial value */0, /* format */NULL);

  stat_reg_counter(sdb, "FMT_il1",
           "FMT: il1 miss penalty cycles",
           &FMT_il1_penalty, /* initial value */0, /* format */NULL);

  stat_reg_counter(sdb, "FMT_il2",
           "FMT: il2 miss penalty cycles",
           &FMT_il2_penalty, /* initial value */0, /* format */NULL);

  stat_reg_counter(sdb, "FMT_dtlb",
           "FMT: dtlb miss penalty cycles",
           &FMT_dtlb_penalty, /* initial value */0, /* format */NULL);

  stat_reg_counter(sdb, "FMT_dl1",
           "FMT: dl1 miss penalty cycles",
           &FMT_dl1_penalty, /* initial value */0, /* format */NULL);

  stat_reg_counter(sdb, "FMT_dl2",
           "FMT: dl2 miss penalty cycles",
           &FMT_dl2_penalty, /* initial value */0, /* format */NULL);

  stat_reg_counter(sdb, "FMT_branch",
           "FMT: branch misprediction penalty cycles",
           &FMT_branch_penalty, /* initial value */0, /* format */NULL);

  stat_reg_counter(sdb, "FMT_long_FU",
      ...
      if (RUU[RUU_head].ea_comp)
	{
      ...
      if (MD_OP_FLAGS(rs->op) & F_CTRL) {
        ...
        /* correctly predicted branch */
          ...
          FMT_itlb_penalty += fmt->local_itlb_penalty;
          FMT_il1_penalty += fmt->local_il1_penalty;
          FMT_il2_penalty += fmt->local_il2_penalty;
        }
	      ...
	      if (MD_OP_FUCLASS(rs->op) != NA)
		{
		  ...
		  if (fu)
			      ...
			      == (F_MEM|F_LOAD)))
			{
              ...
              int l1_miss = FALSE, l2_miss = FALSE, tlb_miss = FALSE;
			  ...
			  if (!load_lat)
			      ...
			      if (cache_dl1 && valid_addr)
				{
                  ...
                  counter_t initial_dl1_misses = cache_dl1->misses;
                  counter_t initial_dl2_misses = cache_dl2->misses;
                  ...
                  /* check L1/L2 cache misses using buffered values */
                  if (initial_dl1_misses != cache_dl1->misses)
                l1_miss = TRUE;
                  if (initial_dl2_misses != cache_dl2->misses)
                l2_miss = TRUE;
				}
			      else
				{
			  ...
			  if (dtlb && MD_VALID_ADDR(rs->addr))
				...
				events |= PEV_TLBMISS;
                  ...
                  /* check TLB miss using buffered values */
  ...
  for (i=0, branch_cnt=0;
	  ...
	  && !(fetch_regs_PC & (sizeof(md_inst_t)-1)))
	{
	  ...
	  if (cache_il1)
	    {
          /* buffer miss counters before access */
          counter_t initial_il1_misses = cache_il1->misses;
          counter_t initial_il2_misses = cache_il2->misses;
		...
		last_inst_missed = TRUE;
          ...
          /* check L1/L2 cache misses using buffered values */
          if (initial_il1_misses != cache_il1->misses)
        l1_miss = TRUE;
          if (initial_il2_misses != cache_il2->misses)
        l2_miss = TRUE;
	    }
	  ...
	  if (itlb)
	    {
          ...
          counter_t initial_itlb_misses = itlb->misses;
...
sim_main(void)
{
  ...
  if (fastfwd_count > 0)
      ...
      for (icount=0; icount < fastfwd_count; icount++)
	{
	  ...
	  /* execute the instruction silently */
...
#include "machine.def"
  ...
  for (;;)
      ...
      if (bugcompat_mode)
	{
      ...
      /* buffer fetch_num value */
      int initial_fetch_num = fetch_num;
	...
	ruu_fetch_issue_delay--;
      ...
      if (RUU_num == RUU_size) {
        ...
        int i=FMT_dispatch_head;
        for (;;) {
          FMT[i].local_branch_penalty++;
          if (i == FMT_dispatch_tail)
            break;
          i = (i + 1) % FMT_SZ;
        }
      }
